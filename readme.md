...
Next, Featuristic scores those formulas. **By default** it uses Pearson correlation with the target, computed in the compiled Nim backend. Those formulas yielding features with the strongest correlations are then selected and recombined using a genetic algorithm to produce offspring, as illustrated below.
